    ...
    if (const SymbolToken *symbol_token_ptr = std::get_if<SymbolToken>(&curr_token)) {
        if (symbol_token_ptr->name_ == "mat") { // we are initializing some variable matrix
            ...
            if (expr.size() == 1) {
                ...
                As<CommandObject>(object)->AddArg(std::make_shared<ExpressionObject>(std::move(expr)));
            } else {
        ...
        if (depth > 0) {
            object = std::make_shared<Number>(constant_token_ptr->value_);
        } else {
        ...
        while (true) { // reading args of function
            ...
            if (bracket_token_ptr && *bracket_token_ptr == BracketToken::CLOSE) {
                ...
                if (data.size() == 1) {
                    ...
                    As<CommandObject>(object)->AddArg(std::make_shared<ExpressionObject>(std::move(data)));
                } else {
    ...
    while (true) {
        ...
        if (symbol_token_ptr || constant_token_ptr) {
            ...
            if (symbol_token_ptr) {
                ...
                symb_obj = std::make_shared<Number>(constant_token_ptr->value_);
            }
            tokenizer->Next();
            curr_token = tokenizer->GetToken();
            if (const SymbolToken *token_ptr = std::get_if<SymbolToken>(&curr_token)) {
...
std::shared_ptr<Object> ReadMatrix(Tokenizer *tokenizer) {
    ...
    return std::make_shared<MatrixObject>(std::move(objects));
}
    ...
    while (true) {
        ...
        if (const BracketToken *bracket_token_ptr = std::get_if<BracketToken>(&curr_token)) {
            ...
            if (data.size() == 1) {
                ...
                objects.push_back(std::make_shared<ExpressionObject>(std::move(data)));
            } else {
